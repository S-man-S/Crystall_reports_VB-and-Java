...
public class CRJavaHelper {
                                         ...
                                         String jndiName) throws ReportSDKException
    {
        ...
        if (subreportName == null || subreportName.equals (""))
        {
            ...
            oldConnection.setConnectionInfo(tables.getTable (0).getConnectionInfo());
            Connection newConnection = new Connection();
            newConnection.setConnectionInfo(connectionInfo);
            clientDoc.getDatabaseController().replaceConnection(oldConnection, newConnection, DBOptions._doNotVerifyDB|DBOptions._ignoreCurrentTableQualifiers);
        }
        ...
        if (subreportName == null || !(subreportName.equals ("")))
        {
            ...
            for (int subNum = 0; subNum < subNames.size (); subNum++)
            {
                ...
                if(tables.size()>0){
                    ...
                    Connection newConnection = new Connection();
                    newConnection.setConnectionInfo(connectionInfo);
                    clientDoc.getSubreportController().getSubreport(subNames.getString(subNum)).getDatabaseController().replaceConnection(oldConnection, newConnection, DBOptions._doNotVerifyDB|DBOptions._ignoreCurrentTableQualifiers);
                }
            }
        }
    }
